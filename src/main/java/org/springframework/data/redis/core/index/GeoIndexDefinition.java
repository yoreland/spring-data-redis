		...
		public Point convert(Object source) {
			...
			if (ObjectUtils.isArray(source)) {
				...
				if (values.length == 2) {
					...
					Assert.isAssignable(Number.class, values[0].getClass());
					Assert.isAssignable(Number.class, values[1].getClass());

					return new Point(((Number) values[0]).doubleValue(), ((Number) values[0]).doubleValue());
				}
			}

			throw new IllegalArgumentException(String.format("Cannot convert %s to %s.", source.getClass(), Point.class));
		}
